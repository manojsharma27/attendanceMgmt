                    ...
                    for (int i = 0; i < listPermissionsNeeded.size(); i++) {
                        if (perms.get(listPermissionsNeeded.get(i)) != PackageManager.PERMISSION_GRANTED) {
                            if (ActivityCompat.shouldShowRequestPermissionRationale(current_activity, listPermissionsNeeded.get(i)))
                                ...
                                // open location settings page to allow user to turn on GPS
                                if (listPermissionsNeeded.get(i).equals(Manifest.permission.ACCESS_COARSE_LOCATION) ||
                                        listPermissionsNeeded.get(i).equals(Manifest.permission.ACCESS_FINE_LOCATION)) {
                                    Utility.showGpsNotEnabledDialog(activity);
                                    return;
                                }
